...
class AccountTax(models.Model):
    ...
    _description = 'Herencia para agregar informacion de impuesto para webpos'

    ITBIS_TIPOS = {'0', '1', '2', '3', '5', '6'}
    # tipo 0 is the default for non-ITBIS taxes (ISC, etc.); only graded ITBIS tipos need ITBIS group
    ITBIS_GROUP_REQUIRED_TIPOS = {'1', '2', '3', '5', '6'}
    ...
    def _webpos_configuration_issues(self):
        ...
        # Retenciones (tasa negativa): no usan tipo_impuesto_webpos en WebPOS.
        if self._webpos_is_withholding():
            return issues
        ...
        if self._webpos_is_itbis_group() and not self._webpos_is_withholding():
            if amount == 0.0 and tipo not in ('0', '3'):
                ...
                issues.append(
                    ...
                    % {
                        'tipo': tipo,
                        'name': dict(self._fields['tipo_impuesto_webpos'].selection).get(tipo),
                    }
                )
            ...
            elif abs(amount - 1.8) < 0.01 and tipo not in ('1', '5'):
                issues.append(
                    _(
                        'ITBIS 1.8%% debe usar tipo WebPOS 1 (ITBIS 1) '
                        'o 5 (18%% + 10%% ley), no %(tipo)s'
                    )
                    % {'tipo': tipo}
                )
            elif (
                amount not in (0.0,)
                and abs(amount - 18.0) >= 0.01
                and abs(amount - 16.0) >= 0.01
                and abs(amount - 1.8) >= 0.01
            ):
                issues.append(
                    _(
                        'tasa ITBIS %(amount)s%% no estándar (18/16/0/1.8); '
                        'asigne tipo_impuesto_webpos manualmente'
                    )
                    % {'amount': amount}
                )

        if (
            tipo in self.ITBIS_GROUP_REQUIRED_TIPOS
            and not self._webpos_is_itbis_group()
            and not self._webpos_is_withholding()
        ):
            issues.append(
        ...
        if not issues and tipo == '0' and self._webpos_is_itbis_group() and abs(amount) >= 0.01 and not self._webpos_is_withholding():
            issues.append(
